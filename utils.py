...
Loads 6 artifacts (all produced by train_pipeline.py from the notebook):
    traffic_volume_model.pkl (or its .partNN chunks)  the fitted regressor
...
def _ensure_model_file(artifact_dir: str) -> str:
    """The trained Random Forest is too large for a single GitHub file
    (~110 MB even compressed), so train_pipeline.py splits it into
    traffic_volume_model.pkl.part00, .part01, ... (<=80 MB each).
    Reassemble them into the whole traffic_volume_model.pkl the first time
    it's needed, then reuse that file on every later run (skips the join
    step once it already exists)."""
    model_path = os.path.join(artifact_dir, "traffic_volume_model.pkl")
    if os.path.exists(model_path):
        return model_path

    parts = sorted(
        p for p in os.listdir(artifact_dir)
        if p.startswith("traffic_volume_model.pkl.part")
    )
    if not parts:
        raise FileNotFoundError(
            "Neither traffic_volume_model.pkl nor any "
            "traffic_volume_model.pkl.partNN chunks were found in "
            f"{artifact_dir}. Run train_pipeline.py to generate them."
        )
    with open(model_path, "wb") as out_f:
        for part in parts:
            with open(os.path.join(artifact_dir, part), "rb") as in_f:
...
def load_artifacts(artifact_dir: str = ARTIFACT_DIR):
    ...
    model = joblib.load(_ensure_model_file(artifact_dir))
