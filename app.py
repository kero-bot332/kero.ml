...
Requires (same folder):
    - models/traffic_volume_model.pkl      (from the notebook's Section 15,
                                              "Final Model Export" — contains the
                                              final model, the RobustScaler, and
                                              every feature name/category needed
                                              to reproduce its exact preprocessing)
...
Packages:
    pip install streamlit plotly folium streamlit-folium joblib pandas numpy
"""
...
MODEL_PATH = os.path.join("models", "traffic_volume_model.pkl")
...
# encoding / scaling in Sections 5-8), so the EDA stats and the Low/Medium/High
# congestion thresholds (LOW_T, HIGH_T) computed below need to come from the
# same cleaned data the model actually saw — otherwise the dashboard's numbers
# won't quite match what the model was trained/evaluated on.
...
REQUIRED_BUNDLE_KEYS = [
    "model", "scaler", "numeric_features", "feature_columns",
    "weather_categories", "time_of_day_categories",
]
...
def load_model_bundle(path: str):
    ...
    if missing:
        st.error(
            f"`{path}` is missing the key(s) {missing}. This usually means it was "
            "saved by an older version of the notebook's Section 15 (Final Model "
            "Export) cell. Delete this file and re-run "
            "`Traffic_Volume_Project_Restructured.ipynb` from top to bottom "
            "(Restart Kernel → Run All) to regenerate it with the current bundle "
            "format."
        )
        st.stop()
    return (
        ...
        bundle["weather_categories"],
        bundle["time_of_day_categories"],
        bundle.get("best_model_name", "Model"),
        bundle.get("model_results", []),
    )
...
if not os.path.exists(MODEL_PATH):
    st.error(
        f"Model file `{MODEL_PATH}` not found. Run every cell of "
        "`Traffic_Volume_Project_Restructured.ipynb` (Section 15: Final Model "
        "Export) first — it creates the `models/` folder and saves the bundle there."
    )
    st.stop()
...
(model, scaler, numeric_features, feature_columns, weather_categories,
 time_of_day_categories, best_model_name, model_results) = load_model_bundle(MODEL_PATH)
...
def predict_traffic(hour, day_of_week, month, temp_c, rain_1h, snow_1h,
    ...
    # Recreate the one-hot columns for weather_main / time_of_day exactly as
    # they appear in feature_columns (drop_first=True was used when training).
    ...
    return max(0, model.predict(row_df)[0])
...
def delay_minutes(volume):
    return int(round(np.interp(volume, [MIN_VOLUME, MAX_VOLUME], [0, 30])))


def congestion_pct(volume):
    return float(np.clip(volume / MAX_VOLUME * 100, 0, 100))
...
with st.sidebar:
    ...
    <div class="card" style="margin-bottom:0;">
        ...
        <div style="display:flex; justify-content:space-between; font-size:12px; color:{TEXT_SUB}; margin-bottom:10px;">
            ...
            <span>Features<br><b style="color:{TEXT_MAIN}; font-size:14px;">{len(feature_columns)}</b></span>
        </div>
