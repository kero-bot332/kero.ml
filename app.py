...
Requires (same folder — all produced by train_pipeline.py from the notebook):
    - traffic_volume_model.pkl (or .partNN chunks)  the fitted regressor
...
# ---------------------------------------------------------------------------
REQUIRED_ARTIFACTS = [
    "scaler.pkl", "power_transformer.pkl",
    "features.pkl", "transform_cols.pkl", "target_info.pkl",
]


def _model_present(artifact_dir: str) -> bool:
    # The trained model may be a single traffic_volume_model.pkl, or split
    # into traffic_volume_model.pkl.part00, .part01, ... (see
    # train_pipeline.py / utils._ensure_model_file) to stay under GitHub's
    # 100 MB per-file limit.
    if os.path.exists(os.path.join(artifact_dir, "traffic_volume_model.pkl")):
        return True
    return any(
        f.startswith("traffic_volume_model.pkl.part")
...
def load_model_artifacts(artifact_dir: str):
    ...
    if not _model_present(artifact_dir):
        missing.append("traffic_volume_model.pkl (or its .partNN chunks)")
    if missing:
        st.error(
